...
init_excel <- function() {
  ...
  # Create the "entries" sheet (analogous to the "entries" table)
  # The column headers will be the same as your SQL table definition
  entry_columns <- c("id", "user_name", "applicant_code", "school1_id", "school2_id", "school3_id", "user_prov", "user_mun", "user_level", "timestamp")
  addWorksheet(wb, "entries")
  writeData(wb, sheet = "entries", x = data.frame(matrix(ncol = length(entry_columns), nrow = 0, dimnames = list(NULL, entry_columns))))
  
  # Create the "school_counts" sheet (analogous to the "school_counts" table)
  school_count_columns <- c("school_id", "school", "municipality", "number_applicants", "first_pref_schools")
  addWorksheet(wb, "school_counts")
  
  # Populate the school_counts sheet with data from your 'school' dataframe
  # The 'school' dataframe is assumed to be available
  school_data <- data.frame(
    school_id = school$SchoolID,
    school = school$SchoolName,
    municipality = school$Municipality,
    number_applicants = 0,
    first_pref_schools = 0
  )
  writeData(wb, sheet = "school_counts", x = school_data)
...
ui <- fluidPage(
  ...
  #More explicit theming to fix modal
  tags$head(
    tags$style(HTML("
...
server <- function(input, output, session) {
  ...
  output$STRIDE3 <- renderUI({
    shinyauthr::logoutUI(
      id = "logout",
      label = "Log Out",
      class = "btn-danger" # Bootstrap button styling 
    )# Custom styling
...
<br/>Email Address: spims@dmw.gov.ph")
  ...
  output$STRIDE4 <- renderUI({
    ...
    observeEvent(input$applicant_code, {
      ...
      conn <- dbConnect(RSQLite::SQLite(), xlsx_path)
    ...
    observeEvent(input$school2, {
      ...
      updateSelectInput(session, "school3", choices = remaining_for_3, selected = "")
        ...
        get_db_conn <- function() {
          dbConnect(RSQLite::SQLite(), xlsx_path)
        }
        # Submission logic
    ...
    ")
        ...
        output$submission_number <- renderValueBox({
          # Connect to the SQLite database using the same xlsx_path as init_db
          conn <- dbConnect(RSQLite::SQLite(), xlsx_path)
